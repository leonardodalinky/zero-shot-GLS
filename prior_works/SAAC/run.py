...
def parse_args():
    ...
    ###################
    parser.add_argument(
        "-precision", type=int, default=26, help="precision for arithmetic encoding method"
    )
    parser.add_argument(
        "-temp", type=float, default=1.0, help="temperature for arithemtic/huffman encoding method"
    )
    parser.add_argument("-topK", type=int, default=50, help="topK for arithemtic encoding method")
    parser.add_argument(
        "-nucleus", type=float, default=0.95, help="neclues for adaptive arithemtic encoding method"
    )
    parser.add_argument(
        "-delta", type=float, default=0.01, help="delta for adaptive arithemtic encoding method"
    )
    ####################
    #                  #
    #    validating    #
...
def encrypt(
    ...
    with prompt_gen.random_state(seed):
        ...
        output_ids, avg_nll, used_bits = encode_saac(
            model=model,
            enc=tokenizer,
            message=message,
            context=context_ids,
...
if __name__ == "__main__":
    ...
    with open(args.output, "w") as fp, prompt_gen.gen_prompt_ctx(
        ...
        for row_idx, row in enumerate(
            tqdm(input_data[start_idx:end_idx], desc="SAAC-Bits-To-Stego", dynamic_ncols=True)
        ):
            seed = seeds[row_idx]
