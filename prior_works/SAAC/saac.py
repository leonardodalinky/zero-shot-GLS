...
def encode_saac(
    ...
    with torch.no_grad():
        ...
        while i < len(message) or (finish_sent and not sent_finish):
            ...
            log2_probs = torch.log2(F.softmax(logits, dim=0))
...
def decode_saac(
    model, enc, text, context, device="cuda", temp=1.0, precision=16, topk=50000, nucleus=0.95
):
    ...
    with torch.no_grad():
        ...
        while i < len(inp):
            logits, past = model(
                prev.unsqueeze(0), past=past
            )  # logits of size (1, sequence_length, hidden_size),
            past = limit_past(past)
