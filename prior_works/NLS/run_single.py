...
from block_baseline import decode_block, encode_block, get_bins
from huffman_baseline import decode_huffman, encode_huffman
...
"""
    ...
    if mode == "arithmetic":
        ...
        out, nll, kl, words_per_bit = encode_huffman(
            model, enc, message, context_tokens, block_size, finish_sent=finish_sent
        )
    elif mode == "bins":
    ...
    if mode != "sample":
        if mode == "arithmetic":
            message_rec = decode_arithmetic(
                model, enc, text, context_tokens, temp=temp, precision=precision, topk=topk
            )
        elif mode == "huffman":
            message_rec = decode_huffman(model, enc, text, context_tokens, block_size)
        elif mode == "bins":
            message_rec = decode_block(
                model, enc, text, context_tokens, block_size, bin2words, words2bin
            )
        ...
        if unicode_enc:
            ...
            reconst = encode_arithmetic(
                model, enc, message_rec, message_ctx, precision=40, topk=60000
            )
