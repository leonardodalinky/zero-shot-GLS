...
class MyData(data.Dataset):
	...
	def __init__(self, text_field, label_field, cover_path=None,
		...
		if examples is None:
			...
			examples += [data.Example.fromlist([line, 'negative'], \
											   fields) for line in cover_path]

			examples += [data.Example.fromlist([line, 'positive'], \
											   fields) for line in stego_path]
	...
	def split(cls, text_field, label_field, args, state, \
		...
		train_cover, test_cover, train_stego, test_stego = \
			ms.train_test_split(df["plaintext"], df["stegotext"], test_size=0.25, random_state=7)

		if state is 'train':
			print('loading the training data...')
			# cover_path = args.train_cover_dir
			# stego_path = args.train_stego_dir
			examples = cls(text_field, label_field, cover_path=train_cover,
						stego_path=train_stego).examples
		...
		if state is 'test':
			...
			examples = cls(text_field, label_field, cover_path=test_cover,
						stego_path=test_stego).examples
