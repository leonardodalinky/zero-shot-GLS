...
parser.add_argument(
    "-kernel-num", type=int, default=100, help="number of each kind of kernel [defualt:100]"
)
parser.add_argument(
    "-dropout", type=float, default=0.5, help="the probability for dropout [defualt:0.5]"
)
parser.add_argument(
    "-static", action="store_true", default=False, help="fix the embedding [default:False]"
)
...
if args.test:
    ...
    test_iter = data.Iterator.splits([test_data], batch_sizes=[64], device=args.device, sort=False)[
        0
    ]
